...
use App\{Follow, Product, Shipment, User, UserDetail, FoodBooking, GroceryBooking, StoreBooking};
...
class UserController extends Controller
{
    ...
    {
            $data['user']=User::with('detail')->where('id',$request->user_id)->first();
            $data['no_of_following']=Follow::where('user_id',$request->user_id)->count();
            $data['no_of_follower']=Follow::where('follow_id',$request->user_id)->count();
            $data['list_of_products']=Product::with('images', 'user')->where('vendor_id',$request->user_id)->where('available',1)->where('status',1)->get();
        
        if ($request->user_id==Auth::user()->id) {
        
            // $data['list_of_sold_product']=Shipment::where('user_id',$request->user_id)->get();
       $data['list_of_purchase_product']=Shipment::with('product','product.images','user')->where('user_id',$request->user_id)->get();
        $data['list_of_sold_product']=Shipment::with('product','product.images','product.user')->where('seller_id',$request->user_id)->get();
            
            }
        else
            {
            //     $data['user']=User::with('detail')->where('id',Auth::user()->id)->first();
            //     $data['no_of_following']=Follow::where('user_id',Auth::user()->id)->count();
            //     $data['is_Followed']=Follow::where('user_id',$request->user_id)->count();
            // $data['list_of_products']=Product::with('images', 'user')->where('vendor_id',$request->user_id)->get();
            //     // $data['list_of_sold_product']=Shipment::where('user_id',$request->user_id)->get();
            //   $data['list_of_purchase_product']=Shipment::with('product','product.images','product.user')->where('user_id',$request->user_id)->get();
            //   $data['list_of_sold_product']=Shipment::with('product','product.images','user')->where('seller_id',$request->user_id)->get();
            
            //     return response()->json(compact('data'),200);
        
            $data['is_Followed']=Follow::where('follow_id',$request->user_id)->where('user_id',Auth::user()->id)->count();
            }
    ...
    public function post_seller_account_details (Request $request)
    {
        ...
        try
        {
           $user= UserDetail::where('user_id', $request->user_id)
      ->update(
          [
