...
use App\{GroceryBooking,GroceryBookingDetails, GroceryTransaction,User,Rider};
use Exception;
...
class GroceryBookingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $message = "Grocery Booking is Done Successfully";
        if($validator->fails())
        {
        ...
        try 
        {
            ...
            foreach(json_decode($items) as $key=>$item)
            {
                ...
                $gbd = new GroceryBookingDetails();
                $gbd->grocery_booking_id = (int)$booking->id;
                $gbd->quantity = (int)$item->orderquantity;
                $gbd->grocery_id = (int)$item->id;
                $gbd->vendor_id = (int)$vendor_id;
                $gbd->price = (double)$item->price;
                $gbd->total_price = (double)$total;
                $gbd->save();
            }
            ...
            if(isset($payment_method) && $payment_method == 'card'){
              ...
              $transaction = new GroceryTransaction();
              $transaction->vendor_id = (int)$vendor_id;
              $transaction->user_id = (int)$user_id;
              $transaction->grocery_booking_id = $booking->id;
      ...
      public function completedOrders($user_id)
    {
        try
        {
            ...
            $records = $model->whereHas('details')
                ->where('user_id', $user_id)->whereIn('status', ['rejected', 'completed','vendor_rejected'])->with(['details.grocery','details.vendor' => function($q){
    ...
    public function inProgressOrders($user_id)
    {
        try
        {
            ...
            $model = new GroceryBooking();
            $records = $model->whereHas('details')
                ->where('user_id', $user_id)->whereIn('status', ['pending', 'accepted','vendor_accepted','pickup'])->with(['details.grocery', 'details.vendor' => function($q){
                $q->with(['detail' => function($q){
                    $q->select('user_id', 'image');
                }])->with('locations');
            }])->latest()->get();
    ...
    public function inProgressOrdersVendor($vendorId)
    {
        try
        {
            ...
            $records = $groceryModel->whereIn('status', ['pending', 'accepted','vendor_accepted','pickup'])->whereHas('details', function($q) use($vendorId){
                $q->where('vendor_id', $vendorId);
            })->with(['details.grocery', 'user.detail'])->latest()->get();
    ...
    public function completedOrdersVendor($vendorId)
    {
        ...
        {
            $groceryModel = new GroceryBooking();
            $records = $groceryModel->whereIn('status', ['rejected', 'completed','vendor_rejected'])->whereHas('details', function($q) use($vendorId){
                $q->where('vendor_id', $vendorId);
            })->with(['details.grocery', 'user.detail'])->latest()->get();
