...
use App\{FoodBooking,FoodBookingDetail,FoodTransaction,User,Rider};
use Exception;
use DB;
use Kreait\Firebase;
...
class FoodBookingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $message = "Food Booking Is done Successfully";
        if($validator->fails())
        {
        ...
        {   
            ...
            foreach(json_decode($items) as $key=>$item)
            {
                ...
                $fbd = new FoodBookingDetail();
                $fbd->food_booking_id = (int)$booking->id;
                $fbd->quantity = (int)$item->orderquantity;
                $fbd->food_id = (int)$item->id;
                $fbd->vendor_id = (int)$vendor_id;
                $fbd->price = (double)$item->price;
                $fbd->total_price = (double)$total;
                $fbd->save();
            ...
            if(isset($payment_method) && $payment_method == 'card'){
              ...
              $transaction->food_booking_id = (int)$booking->id;
   ...
   public function completedOrders($user_id)
    {
        try
        {
            ...
            $records = $model->whereHas('details')
                ->where('user_id', $user_id)->whereIn('status', ['rejected', 'completed','vendor_rejected'])->with(['details.food','details.vendor'=>function($q){
    ...
    public function inProgressOrders($user_id)
    {
        try
        {
            ...
            $records = $model->whereHas('details')          
            ->where('user_id', $user_id)->whereIn('status', ['pending','accepted','vendor_accepted','pickup'])->with(['details.food', 'details.vendor' => function($q){
                $q->with(['detail' => function($q){
                $q->select('user_id', 'image');
                }])->with('locations');
            }])->latest()->get();
    ...
    public function inProgressOrdersVendor($vendorId)
    {
        try
        {
            ...
            $records = $foodModel->whereIn('status', ['pending', 'vendor_accepted','accepted','pickup'])->whereHas('details', function($q) use($vendorId){
                $q->where('vendor_id', $vendorId);
            })->with(['details.food', 'user.detail'])->latest()->get();
    ...
    public function completedOrdersVendor($vendorId)
    {
        ...
        {
            $foodModel = new FoodBooking();
            $records = $foodModel->whereIn('status', ['vendor_rejected','rejected', 'completed'])->whereHas('details', function($q) use($vendorId){
                $q->where('vendor_id', $vendorId);
            })->with(['details.food', 'user.detail'])->latest()->get();
   ...
   public function driverRequest(Request $request)
    {
       try
       {
            ...
            $doubletolatitude=$record->latitude;
            $doubletolat = (double)$doubletolatitude;
            $doubletolongitude=$record->longitude;
            $doubletolong = (double)$doubletolongitude;
            $doublefromlatitude = $record->details[0]->vendor->locations[0]->latitude;
            $doublefromlat = (double)$doublefromlatitude;
            $doublefromlongitude = $record->details[0]->vendor->locations[0]->longitude;
            $doublefromlong= (double)$doublefromlongitude;
            ...
            if(!is_null($record))
            {
                ...
                'ToLatitude'=>$doubletolat,
                'ToLongitude'=>$doubletolong,
                'FromAddress'=>$record->details[0]->vendor->locations[0]->address,
                'FromLatitude'=>$doublefromlat,
                'FromLongitude'=>$doublefromlong,
                'price'=>$record->delivery_charges,
                'RequestId'=> $key,
                'RideDate'=> $date,
                'RideTime'=>$time,
                'ServiceBudget'=> "0",
   ...
   public function driverstatus(Request $request)
           ...
           $validator = Validator::make($request->all(),[
            ...
            $record = $model->with(['details.vendor'=>function($q)
             ...
             $rider = $ridermodel->where('firebase_id',$rider_id)->with('user')->first();
                // $ridermodel = new User();
                // $rider = $ridermodel->whereIn('role',['rider'])->with('rider',function($p) use($driver_id)
                // {
                //     $p->where('firebase_id',$driver_id)->with('user');
                // })->first();
                // print_r($driver->name);
                // exit;
                // $record = $model->with(['details','details.vendor'=>function($q){
                //     $q->with('detail','locations');
                // },'user'=>function($p) use($driver_id)
                // {
                //     $p->whereIn('role', ['rider'])->where('id',$driver_id)->get();
                // }])->find($booking_id);
    ...
    public function vendorStatus(Request $request)
    {
        try
        {
            ...
            {
                return response()->json(['error'=> 'invalid vendor_id'], $this->successStatus);
